...
.doc-sub   { font-size: 10px; font-weight: 700; color: #0369a1; text-align: right; margin-top: 3px; }
...
.type-re { color: #7e22ce; font-weight: 700; }
.type-rc { color: #0369a1; font-weight: 700; }
.type-rl { color: #b91c1c; font-weight: 700; }
.rt-pos { color: #d97706; font-weight: 700; }
.rt-neg { color: #15803d; font-weight: 700; }
.rt-zero { color: #64748b; font-weight: 700; }
...
<table class="top"><tr>
    ...
    <td style="width:45%; vertical-align:top; text-align:right">
        <div class="doc-label">GRAND LIVRE FOURNISSEUR</div>
...
<table class="bts"><tr>
    <td style="width:48%; padding-right:20px">
        ...
        <div class="party-detail">
            @if(isset($supplier->email) && $supplier->email){{ $supplier->email }}<br>@endif
            @if(isset($supplier->telephone) && $supplier->telephone)Tél : {{ $supplier->telephone }}<br>@endif
            @if(isset($supplier->ville) && $supplier->ville){{ $supplier->ville }}<br>@endif
        </div>
    </td>
    <td style="width:52%">
        ...
        <table class="sum-box" style="width:100%; margin-top:4px">
            ...
            <tr><td>
                <table class="sum-row" style="width:100%"><tr>
                    <td class="sum-l">{{ (!empty($dateFrom) || !empty($dateTo)) ? 'Achats (période)' : 'Total achats (F)' }}</td>
                    <td class="sum-r">{{ number_format($totalF, 2, ',', ' ') }} MAD</td>
                </tr></table>
            </td></tr>
            <tr><td>
                <table class="sum-row" style="width:100%"><tr>
                    <td class="sum-l">{{ (!empty($dateFrom) || !empty($dateTo)) ? 'Retours remb. (période)' : 'Total retours remboursés (R)' }}</td>
...
<table class="ledger">
    <thead><tr>
        <th style="width:160px">Date &amp; Heure / Type</th>
        ...
        <th class="r" style="width:90px">Solde cumulé</th>
    </tr></thead>
    <tbody>
        ...
        <tr style="background:#f8fafc">
            ...
            <td class="r">—</td>
            <td class="r">—</td>
            <td class="r {{ $openingBalance > 0 ? 'rt-pos' : ($openingBalance < 0 ? 'rt-neg' : 'rt-zero') }}">
        ...
        @php
            $typeClass = 'type-f';
            $typeLabel = 'ACHAT';
            if ($row['type'] === 'P') { $typeClass = 'type-p'; $typeLabel = 'PAIEMENT'; }
            elseif ($row['type'] === 'R') {
                if ($row['return_type'] === 'refund')  { $typeClass = 'type-re'; $typeLabel = 'REMB.'; }
                elseif ($row['return_type'] === 'change') { $typeClass = 'type-rc'; $typeLabel = 'ÉCHANGE'; }
                else { $typeClass = 'type-rl'; $typeLabel = 'PERTE'; }
            }
            $isCredit = $row['type'] === 'P' || ($row['type'] === 'R' && $row['return_type'] === 'refund');
            $isNeutral = $row['type'] === 'R' && in_array($row['return_type'], ['change', 'loss']);
        @endphp
        <tr>
            <td class="{{ $typeClass }}" style="font-family:'Courier New',monospace; font-size:8px; white-space:nowrap">
                {{ $row['created_at'] }} / {{ $typeLabel }}
            </td>
            ...
            <td class="r">
                @if($isNeutral)
                    <span style="color:#64748b">{{ number_format($row['total_price'], 2, ',', ' ') }}</span>
                @elseif($isCredit)
                    <span style="color:#15803d">-{{ number_format($row['total_price'], 2, ',', ' ') }}</span>
                @else
                    <span style="color:#1d4ed8">+{{ number_format($row['total_price'], 2, ',', ' ') }}</span>
                @endif
            </td>
            <td class="r {{ $row['running_total'] > 0 ? 'rt-pos' : ($row['running_total'] < 0 ? 'rt-neg' : 'rt-zero') }}">
